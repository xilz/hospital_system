<template>
  ...
  >
    <van-swipe-item v-for="item in homeData.banners" :key="item.id">
      <van-image :src="'http://localhost:3001' + item.image" radius="5" />
    </van-swipe-item>
  </van-swipe>
  <van-notice-bar left-icon="volume-o" :scrollable="false">
    ...
    >
      <van-swipe-item v-for="item in homeData.notices" :key="item.id"
        >{{ item.title }}: {{ item.content }}</van-swipe-item
      >
    </van-swipe>
  </van-notice-bar>

  <van-row justify="space-around" style="margin: 6px 0">
    <van-col
